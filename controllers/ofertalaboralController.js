...
getById = async (req, res) => {
    try {
        const oferta = await OfertaLaboral.getById(req.body);
        if (!oferta) {
...
update = async (req, res) => {
    try {
        const ofertaActualizada = await OfertaLaboral.update(req.body);
...
deleteOfertaLaboral = async (req, res) => {
    try {
        const ofertaEliminada = await OfertaLaboral.delete(req.body);
        if (!ofertaEliminada) {
            return res.status(404).json({ message: "Oferta laboral no encontrada." });
        }
